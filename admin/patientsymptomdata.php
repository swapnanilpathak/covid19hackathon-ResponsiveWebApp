...
<body>
  ...
  <table class="table">
    <tr><th>Full Name</th><th>Address</th><th>District</th><th>pincode</th><th>phone number</th><th>Fever</th><th>Cough</th><th>Respiratory Distress</th><th>Travel History</th><th>Timesubmitted</th><th>status</th></tr>
            <?php 
              $foodSupplyList = $con->prepare("SELECT * FROM patientsymptomdata");

              $foodSupplyList->execute();
              while($row = $foodSupplyList->fetch(PDO::FETCH_ASSOC)){
                echo "<tr><td>".$row['fullname']."</td><td>".$row['address']."</td><td>".$row['district']."</td><td>".$row['pincode']."</td><td>".$row['phoneno']."</td><td>".$row['fever']."</td><td>".$row['cough']."</td><td>".$row['respiratorydistress']."</td><td>".$row['travelhistory']."</td><td>".$row['timesubmitted']."</td><td><form method='POST' action='patientsymptomdata.php'>
