...
<body>
  ...
  <table class="table">
    <tr><th>Full Name</th><th>Age</th><th>Gender</th><th>Address</th><th>City/Town/Village</th><th>District</th><th>pincode</th><th>phone number</th><th>Next Of Kin</th><th>Admission Date Requested</th><th>timesubmitted</th><th>status</th></tr>
            <?php 
              $foodSupplyList = $con->prepare("SELECT * FROM hospitaladmissions");

              $foodSupplyList->execute();
              while($row = $foodSupplyList->fetch(PDO::FETCH_ASSOC)){
                echo "<tr><td>".$row['fullname']."</td><td>".$row['age']."</td><td>".$row['gender']."</td><td>".$row['address']."</td><td>".$row['city']."</td><td>".$row['district']."</td><td>".$row['pincode']."</td><td>".$row['phoneno']."</td><td>".$row['kinname']."</td><td>".$row['admdate']."</td><td>".$row['timesubmitted']."</td><td><form method='POST' action='hospitaladmissions.php'>
