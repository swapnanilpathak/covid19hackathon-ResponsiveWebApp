...
<body>
  ...
  <table class="table">
    <tr><th>Full Name</th><th>Date Requested</th><th>District</th><th>Timesubmitted</th><th>status</th></tr>
            <?php 
              $foodSupplyList = $con->prepare("SELECT * FROM counsellings");

              $foodSupplyList->execute();
              while($row = $foodSupplyList->fetch(PDO::FETCH_ASSOC)){
                echo "<tr><td>".$row['fullname']."</td><td>".$row['cdate']."</td><td>".$row['district']."</td>"."<td>".$row['timesubmitted']."</td><td><form method='POST' action='counsellings.php'>
